...
void imshow(double* SRC)
{
    ...
    for (int i = 0; i < HEIGHT; i++)
    {
        ...
        {
            *p = (unsigned char)clamp(pow((*S++), 0.45) * 255, 0.0, 255.0); p++;// R 通道
            *p = (unsigned char)clamp(pow((*S++), 0.45) * 255, 0.0, 255.0); p++;// G 通道
            *p = (unsigned char)clamp(pow((*S++), 0.45) * 255, 0.0, 255.0); p++;// B 通道
            //*p = (unsigned char)clamp((*S++) * 255, 0.0, 255.0); p++;// R 通道
            //*p = (unsigned char)clamp((*S++) * 255, 0.0, 255.0); p++;// G 通道
            //*p = (unsigned char)clamp((*S++) * 255, 0.0, 255.0); p++;// B 通道
        }
    }
...
int main()
{
    ...
    // 球
    Sphere s1 = Sphere(vec3(-0.6, -0.8, 0.6), 0.2, WHITE);
    Sphere s2 = Sphere(vec3(-0.1, -0.7, 0.2), 0.3, WHITE);
    Sphere s3 = Sphere(vec3(0.5, -0.6, -0.5), 0.4, WHITE);
    ...
    // 底部平面
    shapes.push_back(new Triangle(vec3(1, -1, 1), vec3(-1, -1, -1), vec3(-1, -1, 1), GRAY));
    shapes.push_back(new Triangle(vec3(1, -1, 1), vec3(1, -1, -1), vec3(-1, -1, -1), GRAY));
    // 背部平面
    shapes.push_back(new Triangle(vec3(1, 1, -1), vec3(1, -1, -1), vec3(-1, 1, -1), YELLOW));
    shapes.push_back(new Triangle(vec3(-1, 1, -1), vec3(-1, -1, -1), vec3(1, -1, -1), YELLOW));
    // 左边平面
    shapes.push_back(new Triangle(vec3(-1, 1, 1), vec3(-1, 1, -1), vec3(-1, -1, -1), BLUE));
    shapes.push_back(new Triangle(vec3(-1, 1, 1), vec3(-1, -1, 1), vec3(-1, -1, -1), BLUE));
    // 右边平面
    shapes.push_back(new Triangle(vec3(1, 1, 1), vec3(1, 1, -1), vec3(1, -1, -1), GREEN));
    shapes.push_back(new Triangle(vec3(1, 1, 1), vec3(1, -1, 1), vec3(1, -1, -1), GREEN));
    // 顶部平面
            ...
            for (int j = 0; j < WIDTH; j++)
            {
                ...
                if (res.isHit)
                {
                    ...
                    else
                    {
                        ...
                        randomRay.startPoint = res.hitPoint;
                        randomRay.direction =
                            randomDirection(res.material.normal);
                        // 根据反射率决定光线最终的方向
                        double r = randf();
                        if (r < res.material.specularRate) // 镜面反射
                        {
                            vec3 ref = normalize(reflect(ray.direction,
                                res.material.normal));
                            randomRay.direction = mix(ref, randomRay.direction,
                                res.material.roughness);
                            color = pathTracing(shapes, randomRay, 0);
                        }
                        else if (res.material.specularRate <= r && r <=
                            res.material.refractRate) // 折射
                        {
                            vec3 ref = normalize(refract(ray.direction, res.material.normal, float(res.material.refractAngle)));
                            randomRay.direction = mix(ref, -randomRay.direction, res.material.refractRoughness);
                            color = pathTracing(shapes, randomRay, 0);
                ...
                *p += color.x; p++; // R 通道pow(color.x,0.75);
                *p += color.y; p++; // G 通道pow(color.y,0.75);
                *p += color.z; p++; // B 通道pow(color.z,0.75);
            }
        }
    }
    imshow(image);
